...
ksr_trait <- read_csv("~/Desktop/Current Courses/EEB498/EEB498_IndpProj/CSV/2018_Species_Ontogeny_Trait.csv") %>% 
  ...
  transform(Plot = as.numeric(Plot)) %>% 
  select(c(Plot, Species, `SLA_.cm2.g.`)) %>% 
  group_by(Plot, Species) %>%
  summarise(SLA = mean(`SLA_.cm2.g.`))

ksr_trait_leaf <- read_csv("~/Desktop/Current Courses/EEB498/EEB498_IndpProj/CSV/2018_Species_Ontogeny_Trait.csv") %>% 
  filter(Species != "Lolium_aparine" & Species != "Silene_latifolia") %>% 
  unite(Plot, c(Block, Plot_Number), remove = TRUE, sep = ".") %>% 
  transform(Plot = as.numeric(Plot)) %>% 
  select(c(Plot, Species, `Weight_.g.`)) %>% 
  group_by(Plot, Species) %>%
  summarise(LW = mean(`Weight_.g.`))
...
ksr_trait_sum_leaf <- ksr_trait_leaf %>% 
  group_by(Species) %>% 
  summarise(LW = mean(LW)) %>% 
  filter(Species %in% colnames(ksr_abun_sum)) %>% 
  column_to_rownames(var='Species') %>% 
  data.matrix()
...
ksr_REND_trait_leaf <- read_csv("~/Desktop/Current Courses/EEB498/EEB498_IndpProj/CSV/2018_Species_Ontogeny_Trait.csv") %>% 
  filter(Species != "Lolium_aparine" & Species != "Silene_latifolia") %>% 
  unite(Plot, c(Block, Plot_Number), remove = TRUE, sep = ".") %>% 
  transform(Plot = as.numeric(Plot)) %>% 
  select(c(Plot, Species, `Weight_.g.`)) %>% 
...
ksr_REND_abundance_leaf <- read_csv("~/Desktop/Current Courses/EEB498/EEB498_IndpProj/CSV/2018 2m survey.csv") %>% 
  filter(Size == 2) %>% 
  unite(Plot, c(Block, Plot), remove = TRUE, sep = ".") %>% 
  transform(Plot = as.numeric(Plot)) %>% 
  select(-c(Size, Lactuca_serriola, Phleum_pratense, Phragmites_australis, Silene_latifolia, Sonchus_oleraceous, Symphyothricum_latiflorium)) %>% 
  column_to_rownames(var = 'Plot') %>% 
  select(one_of(unique(ksr_REND_trait_leaf$Species)))
...
cwm_value_func <- function(trait_df, abun_df){
  ...
  return(data.frame(plot_list, bind_rows(list_cwm)[ , 2]) %>% 
    rename("Plot" = "plot_list", "CWM" = "bind_rows.list_cwm....2."))
}

cwm_value_func(trait_df = ksr_trait, abun_df = ksr_abundance)

cwm_value_func(trait_df = ksr_trait_leaf, abun_df = ksr_abundance)
```
...
*FDis is calculated using the entire community species pool*
...
fdis_value_func(trait_df = ksr_trait_sum_leaf, abun_matrix = ksr_abun_sum)
```
...
```{r}
# REND(TPDc = TPDc(TPDs = TPDs(species = ksr_REND_trait$Species, traits = ksr_REND_trait[ , 3], trait_ranges = c(0, Inf)), sampUnit = ksr_REND_abundance))
# 
# REND(TPDc = TPDc(TPDs = TPDs(species = ksr_REND_trait_leaf$Species, traits = ksr_REND_trait_leaf[ , 3], trait_ranges = c(0, Inf)), sampUnit = ksr_REND_abundance_leaf))
```

```{r}
fric_value_func <- function(trait_df, abun_matrix){
  fric_value_list <- REND(TPDc = TPDc(TPDs = TPDs(species = trait_df$Species, traits = trait_df[ , 3]), sampUnit = abun_matrix))
...
#fric_value_func(trait_df = ksr_REND_trait_leaf, abun_matrix = ksr_REND_abundance_leaf)

feve_value_func <- function(trait_df, abun_matrix){
  feve_value_list <- REND(TPDc = TPDc(TPDs = TPDs(species = trait_df$Species, traits = trait_df[ , 3]), sampUnit = abun_matrix))
...
#feve_value_func(trait_df = ksr_REND_trait_leaf, abun_matrix = ksr_REND_abundance)
```
...
The following function utilizes the rpois and the rbinom functions to create an abundance distribution for each species. The lambda for the Poisson distribution is the original abundance (vector) for each species. The size for the binomial distribution is the original abundance (vector) for each species.

```{r}
errant_abun_func <- function(abun_matrix, method){
  ...
  for(i in seq_along(species_list)){
    if(method == "Poisson"){
      ...
      binom_abun <- rbinom(n = nrow(abun_matrix), size = abun_matrix[ , i], prob = 0.8)
...
```{r}
cwm_exp_df_func <- function(trait_df, abun_df, abun_matrix, method, n){
  true_cwm <- cwm_value_func(trait_df = trait_df, abun_df = abun_df)
  list_errant_cwm <- list()
  repeat_list <- c(1:n)
  for(i in seq_along(repeat_list)){
    errant_cwm <- cwm_value_func(trait_df = trait_df, abun_df = errant_abun_func(abun_matrix = abun_matrix, method = method)) %>% select(CWM)
...
set.seed(2501)
cwm_exp_pois_df_sla <- cwm_exp_df_func(trait_df = ksr_trait, abun_df = ksr_abundance, abun_matrix = ksr_abun_matrix, method = "Poisson", n = 100)
set.seed(2501)
cwm_exp_pois_df_lw <- cwm_exp_df_func(trait_df = ksr_trait_leaf, abun_df = ksr_abundance, abun_matrix = ksr_abun_matrix, method = "Poisson", n = 100)

# Errant binomial distribution
set.seed(2501)
cwm_exp_binom_df_sla <- cwm_exp_df_func(trait_df = ksr_trait, abun_df = ksr_abundance, abun_matrix = ksr_abun_matrix, method = "binomial", n = 100)
set.seed(2501)
cwm_exp_binom_df_lw <- cwm_exp_df_func(trait_df = ksr_trait_leaf, abun_df = ksr_abundance, abun_matrix = ksr_abun_matrix, method = "binomial", n = 100)
```

### Matrix multiplication (CWM)

```{r}
cwm_exp_mm_df_func <- function(trait_df, abun_df, abun_matrix, method, n){
  true_cwm <- cwm_value_mm_func(trait_df = trait_df, abun_df = abun_df)
  list_errant_cwm <- list()
  repeat_list <- c(1:n)
  for(i in seq_along(repeat_list)){
    errant_cwm <- cwm_value_mm_func(trait_df = trait_df, abun_df = errant_abun_func(abun_matrix = abun_matrix, method = method)) %>% select(CWM)
    list_errant_cwm[[i]] <- errant_cwm
  }
  errant_df <- map_dfc(list_errant_cwm, as.list)
  names(errant_df)[] <- unlist(mapply(function(x, y) paste(x, seq(1, y), sep = "_"), "Errant_CWM", n))
...
```{r}
scatter_cwm_poisson_sla <- ggplot(data = cwm_exp_pois_df_sla, aes(x = True_CWM, y = Mean_Errant_CWM))+
  geom_point()+
  ...
  labs(x = "True CWM", y = "Mean Errant CWM", title = "Scatterplot of true CWM values against errant CWM values
       constructed by random Poisson distributions (SLA)")

ggsave("/Users/John/Desktop/Current Courses/EEB498/EEB498_IndpProj/Plots/scatter_cwm_poisson_sla.png", plot = scatter_cwm_poisson_sla, dpi = 300, width = par("din")[1], height = par("din")[2])

scatter_cwm_poisson_lw <- ggplot(data = cwm_exp_pois_df_lw, aes(x = True_CWM, y = Mean_Errant_CWM))+
  geom_point()+
  geom_errorbar(aes(ymin = Mean_Errant_CWM - SD_Errant_CWM, ymax = Mean_Errant_CWM + SD_Errant_CWM))+
  geom_abline(slope = 1)+
  labs(x = "True CWM", y = "Mean Errant CWM", title = "Scatterplot of true CWM values against errant CWM values
       constructed by random Poisson distributions (LW)")

ggsave("/Users/John/Desktop/Current Courses/EEB498/EEB498_IndpProj/Plots/scatter_cwm_poisson_lw.png", plot = scatter_cwm_poisson_lw, dpi = 300, width = par("din")[1], height = par("din")[2])

scatter_cwm_binomial_sla <- ggplot(data = cwm_exp_binom_df_sla, aes(x = True_CWM, y = Mean_Errant_CWM))+
  geom_point()+
  geom_errorbar(aes(ymin = Mean_Errant_CWM - SD_Errant_CWM, ymax = Mean_Errant_CWM + SD_Errant_CWM))+
  geom_abline(slope = 1)+
  labs(x = "True CWM", y = "Mean Errant CWM", title = "Scatterplot of true CWM values against errant CWM values
       constructed by random binomial distributions (prob = 0.8) (SLA)")

ggsave("/Users/John/Desktop/Current Courses/EEB498/EEB498_IndpProj/Plots/scatter_cwm_binomial_sla.png", plot = scatter_cwm_binomial_sla, dpi = 300, width = par("din")[1], height = par("din")[2])

scatter_cwm_binomial_lw <- ggplot(data = cwm_exp_binom_df_lw, aes(x = True_CWM, y = Mean_Errant_CWM))+
  geom_point()+
  geom_errorbar(aes(ymin = Mean_Errant_CWM - SD_Errant_CWM, ymax = Mean_Errant_CWM + SD_Errant_CWM))+
  geom_abline(slope = 1)+
  labs(x = "True CWM", y = "Mean Errant CWM", title = "Scatterplot of true CWM values against errant CWM values
       constructed by random binomial distributions (prob = 0.8) (LW)")

ggsave("/Users/John/Desktop/Current Courses/EEB498/EEB498_IndpProj/Plots/scatter_cwm_binomial_lw.png", plot = scatter_cwm_binomial_lw, dpi = 300, width = par("din")[1], height = par("din")[2])
```
...
#   cwm_exp_df_func(trait_df = ksr_trait, abun_df = ksr_abundance, abun_matrix = ksr_abun_matrix, method = "Poisson", n = 10),
#   cwm_exp_mm_df_func(trait_df = ksr_trait, abun_df = ksr_abundance, abun_matrix = ksr_abun_matrix, method = "Poisson", n = 10))
...
```{r}
fdis_exp_df_func <- function(trait_df, abun_matrix, method, n){
  true_fdis <- fdis_value_func(trait_df = trait_df, abun_matrix = abun_matrix)
  list_errant_fdis <- list()
  repeat_list <- c(1:n)
  for(i in seq_along(repeat_list)){
    errant_abun <- errant_abun_func(abun_matrix = abun_matrix, method = method) %>% 
...
# Errant Poisson distribution
set.seed(2501)
fdis_exp_pois_df_sla <- fdis_exp_df_func(trait_df = ksr_trait_sum, abun_matrix = ksr_abun_sum, method = "Poisson", n = 100)
set.seed(2501)
fdis_exp_pois_df_lw <- fdis_exp_df_func(trait_df = ksr_trait_sum_leaf, abun_matrix = ksr_abun_sum, method = "Poisson", n = 100)

# Errant binomial distribution
set.seed(2501)
fdis_exp_binom_df_sla <- fdis_exp_df_func(trait_df = ksr_trait_sum, abun_matrix = ksr_abun_sum, method = "binomial", n = 100)
set.seed(2501)
fdis_exp_binom_df_lw <- fdis_exp_df_func(trait_df = ksr_trait_sum_leaf, abun_matrix = ksr_abun_sum, method = "binomial", n = 100)
```

### Visualization (FDis - True vs Errant)

```{r}
scatter_fdis_poisson_sla <- ggplot(data = fdis_exp_pois_df_sla, aes(x = True_FDis, y = Mean_Errant_FDis))+
  geom_point()+
...
ggsave("/Users/John/Desktop/Current Courses/EEB498/EEB498_IndpProj/Plots/scatter_fdis_poisson_sla.png", plot = scatter_fdis_poisson_sla, dpi = 300, width = par("din")[1], height = par("din")[2])

scatter_fdis_poisson_lw <- ggplot(data = fdis_exp_pois_df_lw, aes(x = True_FDis, y = Mean_Errant_FDis))+
  geom_point()+
  geom_errorbar(aes(ymin = Mean_Errant_FDis - SD_Errant_FDis, ymax = Mean_Errant_FDis + SD_Errant_FDis))+
  geom_abline(slope = 1)+
  labs(x = "True FDis", y = "Mean Errant FDis", title = "Scatterplot of true FDis values against errant FDis values
...
ggsave("/Users/John/Desktop/Current Courses/EEB498/EEB498_IndpProj/Plots/scatter_fdis_poisson_lw.png", plot = scatter_fdis_poisson_lw, dpi = 300, width = par("din")[1], height = par("din")[2])

scatter_fdis_binomial_sla <- ggplot(data = fdis_exp_binom_df_sla, aes(x = True_FDis, y = Mean_Errant_FDis))+
  geom_point()+
  geom_errorbar(aes(ymin = Mean_Errant_FDis - SD_Errant_FDis, ymax = Mean_Errant_FDis + SD_Errant_FDis))+
  geom_abline(slope = 1)+
  labs(x = "True FDis", y = "Mean Errant FDis", title = "Scatterplot of true FDis values against errant FDis values
...
ggsave("/Users/John/Desktop/Current Courses/EEB498/EEB498_IndpProj/Plots/scatter_fdis_binomial_sla.png", plot = scatter_fdis_binomial_sla, dpi = 300, width = par("din")[1], height = par("din")[2])

scatter_fdis_binomial_lw <- ggplot(data = fdis_exp_binom_df_lw, aes(x = True_FDis, y = Mean_Errant_FDis))+
  geom_point()+
  geom_errorbar(aes(ymin = Mean_Errant_FDis - SD_Errant_FDis, ymax = Mean_Errant_FDis + SD_Errant_FDis))+
  geom_abline(slope = 1)+
  labs(x = "True FDis", y = "Mean Errant FDis", title = "Scatterplot of true FDis values against errant FDis values
...
ggsave("/Users/John/Desktop/Current Courses/EEB498/EEB498_IndpProj/Plots/scatter_fdis_binomial_lw.png", plot = scatter_fdis_binomial_lw, dpi = 300, width = par("din")[1], height = par("din")[2])
```

## Functional richness (FRic)

```{r message = FALSE}
fric_exp_df_func <- function(trait_df, abun_df, method, n){
  true_fric <- fric_value_func(trait_df = trait_df, abun_matrix = abun_df)
  list_errant_fric <- list()
  repeat_list <- c(1:n)
  for(i in seq_along(repeat_list)){
    errant_fric <- fric_value_func(trait_df = trait_df, abun_matrix = errant_abun_func(abun_matrix = abun_df %>% data.matrix, method = method) %>% 
                                     column_to_rownames(var = "Plot")) %>% 
      select(FRic)
...
# Errant Poisson distribution
set.seed(2501)
fric_exp_pois_df_sla <- fric_exp_df_func(trait_df = ksr_REND_trait, abun_df = ksr_REND_abundance, method = "Poisson", n = 100)

# Errant binomial distribution
set.seed(2501)
fric_exp_binom_df_sla <- fric_exp_df_func(trait_df = ksr_REND_trait, abun_df = ksr_REND_abundance, method = "binomial", n = 100)
```

### Visualization (FRic - True vs Errant)

```{r}
scatter_fric_poisson_sla <- ggplot(data = fric_exp_pois_df_sla, aes(x = True_FRic, y = Mean_Errant_FRic))+
  geom_point()+
...
ggsave("/Users/John/Desktop/Current Courses/EEB498/EEB498_IndpProj/Plots/scatter_fric_poisson_sla.png", plot = scatter_fric_poisson_sla, dpi = 300, width = par("din")[1], height = par("din")[2])

scatter_fric_binomial_sla <- ggplot(data = fric_exp_binom_df_sla, aes(x = True_FRic, y = Mean_Errant_FRic))+
  geom_point()+
  geom_errorbar(aes(ymin = Mean_Errant_FRic - SD_Errant_FRic, ymax = Mean_Errant_FRic + SD_Errant_FRic))+
  geom_abline(slope = 1)+
  labs(x = "True FRic", y = "Mean Errant FRic", title = "Scatterplot of true FRic values against errant FRic values
...
ggsave("/Users/John/Desktop/Current Courses/EEB498/EEB498_IndpProj/Plots/scatter_fric_binomial_sla.png", plot = scatter_fric_binomial_sla, dpi = 300, width = par("din")[1], height = par("din")[2])
```

## Functional evenness (FEve)

```{r message = FALSE}
feve_exp_df_func <- function(trait_df, abun_df, method, n){
  true_feve <- feve_value_func(trait_df = trait_df, abun_matrix = abun_df)
  list_errant_feve <- list()
  repeat_list <- c(1:n)
  for(i in seq_along(repeat_list)){
    errant_feve <- feve_value_func(trait_df = trait_df, abun_matrix = errant_abun_func(abun_matrix = abun_df %>% data.matrix, method = method) %>% 
                                     column_to_rownames(var = "Plot")) %>% 
      select(FEve)
...
# Errant Poisson distribution
set.seed(2501)
feve_exp_pois_df_sla <- feve_exp_df_func(trait_df = ksr_REND_trait, abun_df = ksr_REND_abundance, method = "Poisson", n = 100)

# Errant binomial distribution
set.seed(2501)
feve_exp_binom_df_sla <- feve_exp_df_func(trait_df = ksr_REND_trait, abun_df = ksr_REND_abundance, method = "binomial", n = 100)
```

### Visualization (FEve - True vs Errant)

```{r}
scatter_feve_poisson_sla <- ggplot(data = feve_exp_pois_df_sla, aes(x = True_FEve, y = Mean_Errant_FEve))+
  geom_point()+
...
ggsave("/Users/John/Desktop/Current Courses/EEB498/EEB498_IndpProj/Plots/scatter_feve_poisson_sla.png", plot = scatter_feve_poisson_sla, dpi = 300, width = par("din")[1], height = par("din")[2])

scatter_feve_binomial_sla <- ggplot(data = feve_exp_binom_df_sla, aes(x = True_FEve, y = Mean_Errant_FEve))+
  geom_point()+
  geom_errorbar(aes(ymin = Mean_Errant_FEve - SD_Errant_FEve, ymax = Mean_Errant_FEve + SD_Errant_FEve))+
  geom_abline(slope = 1)+
  labs(x = "True FEve", y = "Mean Errant FEve", title = "Scatterplot of true FEve values against errant FEve values
...
ggsave("/Users/John/Desktop/Current Courses/EEB498/EEB498_IndpProj/Plots/scatter_feve_binomial_sla.png", plot = scatter_feve_binomial_sla, dpi = 300, width = par("din")[1], height = par("din")[2])
```

## Shannon diversity

```{r}
shannon_exp_df_func <- function(abun_df, method, n){
  true_shannon <- shannon_value_func(abun_df = abun_df)
  list_errant_shannon <- list()
  repeat_list <- c(1:n)
  for(i in seq_along(repeat_list)){
    errant_abun <- errant_abun_func(abun_matrix = abun_df %>% column_to_rownames(var = "Plot") %>% data.matrix, method = method) %>% data.frame()
...
# Errant Poisson distribution
set.seed(2501)
shannon_exp_pois_df <- shannon_exp_df_func(abun_df = ksr_abundance, method = "Poisson", n = 100)

# Errant binomial distribution
set.seed(2501)
shannon_exp_binom_df <- shannon_exp_df_func(abun_df = ksr_abundance, method = "binomial", n = 100)
```

### Visualization (Shannon - True vs Errant)

```{r}
scatter_shannon_poisson <- ggplot(data = shannon_exp_pois_df, aes(x = True_Shannon, y = Mean_Errant_Shannon))+
  geom_point()+
...
ggsave("/Users/John/Desktop/Current Courses/EEB498/EEB498_IndpProj/Plots/scatter_shannon_poisson.png", plot = scatter_shannon_poisson, dpi = 300, width = par("din")[1], height = par("din")[2])

scatter_shannon_binomial <- ggplot(data = shannon_exp_binom_df, aes(x = True_Shannon, y = Mean_Errant_Shannon))+
  geom_point()+
  geom_errorbar(aes(ymin = Mean_Errant_Shannon - SD_Errant_Shannon, ymax = Mean_Errant_Shannon + SD_Errant_Shannon))+
  geom_abline(slope = 1)+
  labs(x = "True Shannon", y = "Mean Errant Shannon", title = "Scatterplot of true Shannon values against errant Shannon values
...
ggsave("/Users/John/Desktop/Current Courses/EEB498/EEB498_IndpProj/Plots/scatter_shannon_binomial.png", plot = scatter_shannon_binomial, dpi = 300, width = par("din")[1], height = par("din")[2])
```
